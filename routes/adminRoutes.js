...
adminRoutes.get('/orders', verifyToken, authorizeRoles("admin"), async (req, res) => {
  	try {
...
// Only Admins and Managers can access this route
adminRoutes.put('/orders/:id/status', verifyToken, authorizeRoles("admin"), async (req, res) => {
...
adminRoutes.post('/orders/:id', verifyToken, authorizeRoles("admin"), async (req, res) => {
  	try {
    	const order = await Order.findById(req.params.id);
    	if (!order) {
      		return res.status(404).json({ message: 'Order not found' });
...
// Only Admins and Managers can access this route
adminRoutes.get('/orders/:id', verifyToken, authorizeRoles("admin"), async (req, res) => {
  	try {
...
// adminRoutes.get('/products', verifyToken, authorizeRoles("admin", "manager"), async (req, res) => {
//   try {
...
adminRoutes.post("/products", verifyToken, authorizeRoles("admin"), async (req, res) => {
	try {
...
adminRoutes.post('/registerAdmin', verifyToken, authorizeRoles("admin"), async (req, res, next) => {
    try{
